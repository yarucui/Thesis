\chapter{Direction 1: disentangling format-dependent from format-invariant expression in LLM advice}
...
recovered nearly all of the entropy gap between the forced-choice
baseline and the human community in every model. The remainder of this
chapter establishes that this recovery is mechanism-specific,
cross-model robust, and not explained by sampling temperature;
\cref{sec:d1_vs} reports the precise recovery figures and defines how
recovery is measured.

\Cref{sec:d1_baseline} documents the baseline entropy gap and its
magnitude. \Cref{sec:d1_hypotheses} frames two competing behavioral
hypotheses — format-dependent versus format-invariant expression — and
derives the diagnostic implication of each. \Cref{sec:d1_vs} presents the Verbalized
...
We constructed a corpus of 811 Reddit decision posts spanning education,
...
\begin{figure}[t]
  \centering
  %% \includegraphics[width=\linewidth]{figures/fig4_1_histogram_pair.pdf}
  \todo{fig 4.1: histogram pair, 132-post human risky\_ratio distribution
    (broad, centred near 0.5) vs LLM baseline risky\_rate distribution
    (bimodal at 0 and 1). Caption: ``Human choices are broadly
    distributed; LLM forced-choice outputs collapse to near-certainty.''}
...
\textbf{H$_{\text{format-dep}}$ (Format-dependent expression).} The
model's elicited distribution over the two options is broad under a
format that permits a distribution, but the forced-choice format at
temperature 0.7 systematically discards this breadth, producing a
near-deterministic output. Under this hypothesis, the collapse is an
artifact of the output format; the model can express pluralism when the
format permits. The fix is a prompt-level change that permits the model
to express a distribution rather than commit to a single token. This is
a claim about output behavior under different formats, not about the
model's internal state.

\textbf{H$_{\text{format-inv}}$ (Format-invariant expression).} The
model's expressed distribution stays peaked regardless of output
format — reinforcement learning from human feedback has compressed the
probability mass over advice options into a single modal answer
\citep{kirk-2024-rlhf-diversity}. A theoretical mechanism for this
compression is the typicality bias identified by
\citet{chung-2025-soft-preference}: the KL regulariser in PPO pushes the
reward-maximising policy toward modal completions, systematically
discounting minority-but-admissible answers. Under this hypothesis, the
peakedness is invariant to the query format; no prompt-level
...
H$_{\text{format-dep}}$ is correct, existing frontier models can already
express the pluralism that aligned advice requires, and the fix costs
one line of a prompt. If H$_{\text{format-inv}}$ is correct, the fix
...
H$_{\text{format-dep}}$ is true, this format change should allow the
model to express a broad distribution, recovering most or all of the
entropy gap. If H$_{\text{format-inv}}$ is true, the expressed
distribution is peaked regardless of how the question is asked;
verbalized output should therefore show entropy close to the
forced-choice baseline. Recovering entropy under elicitation does not
establish that a broad distribution pre-existed the query in the model's
internal state; it shows the model can express such a distribution when
the format permits.
...
favoring the format-dependent-expression hypothesis.
...
0.210. Recovery measures the fraction of the baseline-to-human entropy
gap that a condition closes: of the distance between the forced-choice
baseline entropy and the human community's entropy, the proportion the
verbalized condition recovered. Formally, $(H_{\text{VS}} -
H_{\text{baseline}}) / (H_{\text{human}} - H_{\text{baseline}})$, it was
94--101\% across the four models (\cref{sec:d1_crossmodel}); a value
above 100\% means the model's verbalized distribution was slightly more
spread out than the human reference. The result was statistically
robust: paired bootstrap ($n{=}525$ post--model pairs, 10\,000
resamplings) gave $\Delta = -0.235$, 95\% CI $[-0.250, -0.220]$,
$p_{\text{fdr}} < 0.001$.

\begin{figure}[t]
  \centering
  %% \includegraphics[width=\linewidth]{figures/fig4_2_entropy_4panel.pdf}
  \todo{fig 4.2: 4-panel mean entropy per model per condition (baseline,
    vs, dv, hight) with human reference horizontal dashed line. One
    panel per model, share y-axis 0--0.7 nats.}
...
while producing near-zero entropy under forced choice can express a
broad distribution when the format permits, whatever its internal state.
...
The format-dependent-expression pattern holds across four post-trained
models that differ in training procedure, scale, and developer,
...
different RLHF pipelines \citep{ouyang-2022-instructgpt, bai-2022-constitutional} is the scientifically decisive fact: format-dependent
expression appears to be a property of the post-training paradigm, not of
...
than the other three models, so that its expressed distribution is more
diffuse; when the format permits, that diffuseness overshoots the human
reference slightly. Second, the 0.5 percentage-point overshoot is
...
interpretation remains that output format is the dominant driver. The
precise contribution of Constitutional AI is speculative and left for
future work.
...
Chapter 5 tests role-framing effects across the full 811-post corpus,
covering a broader range of consensus levels and providing a more
general view of the alignment gap.

The format-dependent-expression hypothesis is now supported on four
independent grounds. First, mean entropy rose from 0.046 to 0.608 nats under VS —
...
adjustment over 11 comparisons). The format-invariant-expression
hypothesis is provisionally rejected for these four models: a model
whose expressed distribution were genuinely peaked regardless of format
could not recover near-human entropy when a different output format is
used.
